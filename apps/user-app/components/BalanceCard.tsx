...
export const BalanceCard = ({
  ...
  const [clientLocked, setClientLocked] = useState<number | null>(null);

  useEffect(() => {
    console.log("Server-rendered amount:", amount);
    console.log("Server-rendered locked:", locked);
    setClientAmount(amount);
    setClientLocked(locked);
  }, [amount, locked]);

  if (clientAmount === null || clientLocked === null) {
    return (
      <center>
      ...
      <center>
        <div className="flex flex-col space-y-4">
          ...
          <div className="flex justify-between border-b border-slate-300 py-2 text-xl ">
            ...
            <div>{(clientLocked + clientAmount) / 100} INR</div>
          </div>
        </div>
      </center>
    </Card>
